...
\quad \kappa=\frac{\sigma_{\mathrm{Th}}}{m_p}
	...
	\end{equation}
  Below this pressure, our diffusion approximation starts to fail as the mean
  free path length is exceeding the local scale height.
...
leave the fluid element (i.e., the local thermal timescale. We presume that the 
timescale is longer than the time for pressure equilibrium as well:
\begin{multline}
  \label{eq:10}
...
\right|_{\ell=h}\right)^{3/2}
  \end{equation}
  The measure of how effective convection can be is to compare this to the
  radiative heat flux. Recall our result for the radiative flux:
  \begin{equation}
    \label{eq:41}
    F_{\mathrm{rad}}=\frac{1}{3}\frac{c}{\kappa\rho}\frac{d}{dr}\left
		(aT^4\right)\approx \frac{c}{\kappa\rho}\frac{P_{\mathrm{rad}}}{h} =
 		\frac{cP_{\mathrm{rad}}}{\tau}
  \end{equation}
  where in the second equality, we're let $d/dr\to 1/h$, and in the third we've
  used the definition of optical depth, \eqref{HTS.47}, to replace the
  denominator with the optical depth through a scale height. Now we find
...
_{\mathrm{adiabatic}}=\frac{2}{5}
  ...
  \item[1.] \textbf{Fully Convective and Efficient}: $P\propto \rho^{5/3}$,
    where the prefactor is the specific entropy of the star, \textit{which is
...
{3/5}\left(\frac{R_\odot}{R}\right)^{1/5}
  ...
  \end{equation}
  where the free electrons probably come from an alkali metal due to their very
  low first ionization energies. Hansen, Kawaler, and Trimble give the value of
  this opacity as
  \begin{equation}
    \label{eq:137}
...
{keV}
  \end{equation}
  Note that we've been assuming spherically-symmetric and non-rotating geometry.
  However, the radius is shrinking several orders of magnitude, so any small
  amount of rotation is likely balooned by the collapse. So it is hard or 
  impossible to start with such a low specific angular momentum to allow for a 
  spherical collapse. Instead, the accretion onto the protostar is done through
  an accretion disk where angular momentum can be removed through jets or the
  disk itself. That's hard to model\ldots, so we'll return to a fantastical
...
{fm})}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics{figures/energyPlot.pdf}
...
{\mathrm{H}}
    ...
    \begin{equation}
      \label{eq:211}
      \boxed{
      \frac{y^2}{1-y}=\frac{4\times 10^{-9}~\mathrm{g~cm^{-3}}}{\rho}
      \left(\frac{T}{\mathrm{1~K}}\right)^{3/2}
      e^{-\frac{1.6\times 10^5~\mathrm{K}}{T}}
      }
    \end{equation}
    ...
    \end{equation}
    Where we have used the fact that $R\propto M^{0.75}$ on the main 
    sequence. Recalling the optical depth, $\tau=\int \rho\kappa\, dr$, 
    we will say that at the photosphere, $\tau_{\mathrm{ph}}=2/3$. Then 
    we find
    \begin{equation}
      \label{eq:214}
...
\left(\frac{10^4\
          \mathrm{K}}{T}\right)\left(\frac{g}{2.7\times 10^4
          ~\mathrm{cm~s^{-2}}}\right)
    \end{equation}
...
{\mathrm{line}}}\quad\Rightarrow\quad
    ...
    With the Saha equation (or at least something like it), we can 
    compute what fraction of hydrogen atoms are actually ionized into 
    the $\mathrm{H}^-$ ion:
    \begin{equation}
      \label{eq:222}
      \frac{n(\mathrm{H})}{n(\mathrm{H}^-)}=\frac{n_Q}{n_e}\exp\left(-
      \frac{0.75\ \mathrm{eV}}{kT}\right)
    \end{equation}
    or
    \begin{equation}
      \label{eq:223}
      \frac{n(\mathrm{H^-})}{n(\mathrm{H})}=\frac{n_e}{n_Q}\exp\left[
      \frac{8700~\mathrm{K}}{T}\right]
    \end{equation}
    For the sun, this gives $n_e=10^{13}\ \mathrm{cm^{-3}}$ and thus
    \begin{equation}
      \label{eq:224}
      \frac{n(\mathrm{H}^-)}{n(\mathrm{H})}\approx 10^{-8}e^{8700~
      \mathrm{K}/T}
    \end{equation}
...
\frac{\mu^4M^3}{1+X}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width = 4in]{figures/MSDiagram.pdf}
...
\right)^{25/3}\left(\frac{M_c}{0.2M_\odot}\right)^{10}
    ...
    \begin{equation}
    	\label{eq:247a} L_{\mathrm{RGB}} = L_\odot \left(\frac{M_c}
    	{0.1~M_\odot}\right).
    \end{equation}
    \eqref{eq:247}
    is known as the \textbf{Luminosity-Core Mass Relation} and is
    a solution that is available only if the star can evolve to it 
    (i.e., it isn't guaranteed). This state \emph{is} the red giant 
    branch. As hydrogen shell burning continues, the core mass 
    increases, thus increasing the luminosity.\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \begin{figure}[H]
      \centering
      \includegraphics{figures/compPlot.pdf}
...
{core}}^4}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics{figures/TcRhoc}
...
assuiming $\rho_4=1$.}
   ...
   One other important neutrino emission process is \textbf{plasmon
    ...
    \begin{figure}[H]
      \centering
      \includegraphics{figures/neutrinoCool.pdf}
